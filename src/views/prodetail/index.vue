<template>
  <div class="prodetail">
    ...
    <van-swipe :autoplay="3000" @change="onChange">
      <van-swipe-item>
        <img :src="detailList.goods_image" />
      </van-swipe-item>
    ...
    <div class="info">
      <div class="title">
        <div class="price">
          <span class="now">￥{{detailList.goods_price_max}}</span>
          <span class="oldprice">￥{{detailList.line_price_max}}</span>
        </div>
    ...
    <van-action-sheet v-model="showPannel" :title="mode === 'cart' ? '加入购物车' : '立刻购买'">
      <div class="product">
        ...
        <div class="num-box">
          <CountBox v-model="account"></CountBox>
        </div>
...
import { getProDetail, getProDetailTalk } from '@/api/product'
...
export default {
  ...
  methods: {
    async getData () {
      const { data: { detail } } = await getProDetail(this.getGoodsId)
      this.detailList = detail
      // console.log(detail)
    },
    async getComments () {
      const { data: { list, total } } = await getProDetailTalk(this.getGoodsId, this.limit)

      this.commentList = list
      this.total = total
      // console.log(list[0].comment_id)
    },
